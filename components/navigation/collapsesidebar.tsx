...
const MobileSideBar = ({ className, sideNavItemInfo }: MobileSideBarProps) => {
    ...
    const sideNavItems = sideNavItemInfo.map((item, i) => {
        return (
            <SheetClose key={i} asChild>
                <Link href={item.href} className="text-foreground p-2 rounded-lg dark:text-foreground hover:underline group">
                    {item.name}
                </Link>
            </SheetClose>
        )
    ...
    return (
        <div className={`${className} fixed bg-sidebar m-2 items-center bg-white/0 right-0`}>
            <Sheet>
